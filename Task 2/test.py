...
def load_npz_model():
    if not os.path.exists(WEIGHTS_FILE):
        messagebox.showerror("Error", f"'{WEIGHTS_FILE}' ஃபைல் கிடைக்கவில்லை! முதலில் Training Code-ஐ Run செய்து '.npz' கோப்பை உருவாக்கவும்.")
        return None
    try:
        ...
        messagebox.showerror("Error", f"Weights load செய்வதில் பிழை: {str(e)}")
        return None
...
if weights_data is not None:
    ...
    threshold = float(weights_data['optimal_threshold']) if 'optimal_threshold' in weights_data else 0.5
    test_acc = float(weights_data['test_accuracy']) if 'test_accuracy' in weights_data else 0.0
...
def predict_student_status(study_hours, attendance, prev_marks, assignments):
    X_raw = np.array([[study_hours, attendance, prev_marks, assignments]], dtype=np.float64)
...
class StudentPredictorApp:
    ...
    def create_widgets(self):
        ...
        acc_text = f"Accuracy: {test_acc:.1f}%" if test_acc > 0 else "Loaded from model_weights.npz"
        ...
        # Form Card
        form_card = tk.Frame(self.root, bg=self.CARD_BG, bd=0, highlightthickness=1, highlightbackground="#313244")
        ...
        for idx, (label_text, key) in enumerate(inputs_info):
            lbl = tk.Label(form_card, text=label_text, font=("Segoe UI", 10, "bold"), bg=self.CARD_BG, fg=self.TEXT_COLOR, anchor="w")
            lbl.pack(fill="x", padx=20, pady=(10 if idx==0 else 5, 2))
        ...
        # Output Display Card
        self.result_card = tk.Frame(self.root, bg=self.CARD_BG, highlightthickness=1, highlightbackground="#313244")
    ...
    def on_predict(self):
        if weights_data is None:
            messagebox.showerror("Error", "model weight was not properly loaded")
            return
        ...
        try:
            ...
            if not (0 <= sh <= 24 and 0 <= att <= 100 and 0 <= pm <= 100 and 0 <= asgn <= 100):
                messagebox.showwarning("Invalid Range", "give the correct number in input (Hours: 0-24, %: 0-100).")
                return
            ...
            messagebox.showerror("Input Error", "enter number onlu   (Numbers only).")
